...
main = do
  ...
  r1 `shouldEqual`
    ( Left
        ( NonEmptyList
            ( NonEmpty (ErrorAtProperty "a" (TypeMismatch "Int" "Undefined"))
                ( (ErrorAtProperty "b" (TypeMismatch "String" "Undefined"))
                    : (ErrorAtProperty "c" (TypeMismatch "Boolean" "Number"))
                    : (ErrorAtProperty "d" (TypeMismatch "array" "Number"))
                    : Nil
                )
            )
        )
  ...
  r3 `shouldEqual`
    ( Left
        ( NonEmptyList
            ( NonEmpty
                ( ErrorAtProperty "b"
                    (TypeMismatch "Nullable String" "Undefined")
                )
                Nil
            )
        )

...
readEitherImpl f = Left <$> JSON.readImpl f
  <|> Right <$> JSON.readImpl f

readRecordWithEitherJSON ∷
  String → Either Foreign.MultipleErrors RecordWithEither
...
readRecordMisnamedField ∷
  String → Either Foreign.MultipleErrors RecordMisnamedField
...
main = do
  ...
  assert <<< isRight $ readRecordWithEitherJSON
    """{"apple": 1, "banana": "yellow"}"""
